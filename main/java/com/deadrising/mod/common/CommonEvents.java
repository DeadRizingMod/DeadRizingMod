...
public class CommonEvents {
	...
	public void onServerJoin(FMLNetworkEvent.ClientConnectedToServerEvent event) {
		...
		if (event.getManager().getRemoteAddress().toString().split("/")[0].equals("51.79.0.176:25749")) {
			discordHandler.setRichPresence("", "Playing on Undead Island", "cubgcurselogo_png", "CUBG", "US_png", "US");
		}
		if (event.getManager().getRemoteAddress().toString().split("/")[0].equals("cubgau.mrcrayfish.com")) {
			discordHandler.setRichPresence("", "Playing on Kangaroo", "cubgcurselogo_png", "CUBG", "EU_png", "eu");
		}
	}

	@SubscribeEvent
	public void onServerLeave(FMLNetworkEvent.ClientDisconnectionFromServerEvent event) {
		discordHandler.setRichPresence("", "In the Main Menu", "cubgcurselogo_png", "deadrising");
	}
